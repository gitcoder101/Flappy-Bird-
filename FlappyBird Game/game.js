var canvas, ctx, width, height, fgpos = 0, frames = 0, score = 0, best = 0, currentstate, states = {Splash: 0, Game: 1, Score: 2}, okbtn,
bird = {
    x: 80,
    y: 100,
    frame: 0,
    ...
    update: function() {
        ...
        if (currentstate === states.Splash) {
            this.y = height - height * 7/12 + 5 * Math.cos(frames/10);
            this.rotation = 0;
        } else {
...
function render() {
    ...
    if (currentstate === states.Splash) {
        s_splash.draw(ctx, width/2 - s_splash.width/2, height/2 - s_splash.height/2);
    ...
    if (currentstate === states.Score) {
        ...
        if (score >= 15) {
            s_medals.Bronze.draw(ctx, width/2 - 87, height - 298)
        } else if (score >= 30) {
            s_medals.Silver.draw(ctx, width/2 - 87, height - 298)
        } else if (score >= 50) {
            s_medals.Gold.draw(ctx, width/2 - 87, height - 298)
        }
    } else {
        s_numberB.draw(ctx, null, 20, score, width/2);
    }
}
